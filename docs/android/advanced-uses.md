...
## Multi-tunneling via Work Profiles

Android has a strict limitation of allowing only one VPN to run at a time per user profile. To get around this limitation, [Android Work Profiles](https://www.android.com/enterprise/work-profile/) can be used to allow a user to seamlessly run two VPN apps (or two instances of WG Tunnel) at the same time.
...
> **Note:** A root-based kernel mode previously allowed true simultaneous multi-tunneling on a single
> profile, but was removed in v5.0.0. See the [FAQ](/docs/faq#is-kernel-mode-support-planned-for-android-or-desktop)
> for details. The work profile approach above is the current way to achieve a multi-tunnel-like
> experience.
