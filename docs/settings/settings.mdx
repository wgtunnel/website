...
Leverages the device's kernel WireGuard module (root and supported Kernel required). No VPN Service needed, enabling multiple tunnels by default. Best for performance and multi-tunnel setups on rooted devices, but can have reliability drawbacks and supports less app features.

### DNS Settings

Configure DNS resolution for peer endpoints (like resolving "my-server.com" in configs). Options:
...
When enabled, bypasses local LAN traffic (all private IP address blocks) from the kill switch, allowing access to local devices while blocking external leaks.

:warning: **Note**: In rare cases, this can cause leaks depending on your DNS setup so use with caution.

### Global Split Tunneling

Enable this to apply universal split tunneling settings to all tunnels without editing individual tunnels.
...
1. **Logs Check**: Takes precedence. If logs indicate issues (like handshake/UDP failures) within the last 2 minutes, marked unhealthy.
...
When enabled, monitors app logs for health (like detecting handshake/UDP issues). Click in to view live logs; top navigation drawer allows clearing or exporting logs.
